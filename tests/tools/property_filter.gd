...
func test_declare_filter() -> void:
	...
	assert_arr_c(
		...
		ClassUtil.class_get_property_list("Node2D", true).map(
			func(prop_info): return prop_info.name
		)
	)
	...
	assert_arr_c(
		...
		(
			...
			+ ClassUtil.class_get_property_list("CanvasItem", true).map(
				func(prop_info): return prop_info.name
			)
			+ ClassUtil.class_get_property_list("Node", true).map(
				func(prop_info): return prop_info.name
			)
			+ ClassUtil.class_get_property_list("Object", true).map(
				func(prop_info): return prop_info.name
			)
		)
	)
...
func test_declare_blacklist_filter() -> void:
	...
	assert_arr_c(
		...
		(
			...
			+ ["CanvasItem", "Node", "Node2D", "script"]
		)  ## class is also in get_object_list
	)
	#filter.declare_blacklist = ["+Node2D"]
	#assert_eq(Array(filter.filter_on(node)), [])
